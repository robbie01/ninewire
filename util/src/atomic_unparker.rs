// This is mostly just copied from smol-rs/atomic-waker with the Unparker stuff
// replaced with Unparker
...
/// An `AtomicUnparker` can coordinate concurrent notifications with the consumer
...
///
/// A single `AtomicUnparker` may be reused for any number of calls to `register` or
/// `wake`.
///
/// # Memory ordering
///
...
/// use futures::task::{Context, Poll, AtomicUnparker};
/// use std::sync::Arc;
...
/// }
/// ```
pub struct AtomicUnparker {
    state: AtomicUsize,
    waker: UnsafeCell<Option<Unparker>>,
}

// `AtomicUnparker` is a multi-consumer, single-producer transfer cell. The cell
// stores a `Unparker` value produced by calls to `register` and many threads can
// race to take the waker (to wake it) by calling `wake`.
//
// If a new `Unparker` instance is produced by calling `register` before an
// existing one is consumed, then the existing one is overwritten.
//
// While `AtomicUnparker` is single-producer, the implementation ensures memory
...
// the `Unparker` cell. There are two bits that are operated on independently.
...
//
// This case is handled by requiring users of `AtomicUnparker` to call `register`
...
/// A new waker value is being registered with the `AtomicUnparker` cell.
const REGISTERING: usize = 0b01;

/// The waker currently registered with the `AtomicUnparker` cell is being woken.
const WAKING: usize = 0b10;

impl AtomicUnparker {
    /// Create an `AtomicUnparker`.
    pub const fn new() -> Self {
        // Make sure that task is Sync
        #[allow(dead_code)]
        trait AssertSync: Sync {}
        impl AssertSync for Unparker {}
    ...
    /// invoked with a new `Unparker` **each** time they require a wakeup.
    ///
    ...
    /// }
    /// ```
    pub fn register(&self, waker: &Unparker) {
        match self
            .state
            ...
            WAITING => {
                unsafe {
                    ...
                    match *self.waker.get() {
                        Some(ref mut old_waker) => old_waker.clone_from(waker),
    ...
    /// Calls `wake` on the last `Unparker` passed to `register`.
    ///
    ...
    /// Returns the last `Unparker` passed to `register`, so that the user can wake it.
    ///
    ///
    /// Sometimes, just waking the AtomicUnparker is not fine grained enough. This allows the user
    ...
    pub fn take(&self) -> Option<Unparker> {
        ...
        // memory the `AtomicUnparker` is associated with.
...
impl Default for AtomicUnparker {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Debug for AtomicUnparker {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "AtomicUnparker")
    }
}

unsafe impl Send for AtomicUnparker {}
unsafe impl Sync for AtomicUnparker {}
